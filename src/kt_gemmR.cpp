...
if (sum(betas == 0) == p) {
  ...
  if(correction) {
    knp = sin(PIE/2.0*tauVal*(n-double(p)-1.0)/n);
    bic = n * log(1.0 - pow(knp,2.0)) + double(kCor) * log(n);
    bicr = n * log(1.0 - pow(r,2.0)) + double(kCor) * log(n);
    aic = n * log(1.0 - pow(knp,2.0)) + 2.0*double(kCor);
    aicr = n * log(1.0 - pow(r,2.0)) + 2.0*double(kCor);
  } else {
    knp = sin(PIE/2.0*tauVal*(n-double(p)-1.0)/n);
    bic = n * log(1.0 - pow(knp,2.0)) + double(p) * log(n);
    bicr = n * log(1.0 - pow(r,2.0)) + double(p) * log(n);    
    aic = n * log(1.0 - pow(knp,2.0)) + 2.0*double(p);
    aicr = n * log(1.0 - pow(r,2.0)) + 2.0*double(p);    
  }
...
// [[Rcpp::export]]
List gemmFitRcppI(double n, NumericMatrix betas, NumericMatrix data, int p, NumericVector kCor, CharacterVector correction, bool isTauB) {
